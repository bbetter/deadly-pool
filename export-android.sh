...
OUT="${EXPORT_OUT:-export/deadly-pool.apk}"
...
if [ -d "$HOME/.local/share/godot/export_templates" ]; then
  ...
  if [ ! -f "$XDG_DATA_HOME/godot/export_templates/4.6.stable/android_release.apk" ]; then
    cp -a "$HOME/.local/share/godot/export_templates/." "$XDG_DATA_HOME/godot/export_templates/"
  fi
fi
# Copy editor settings so Godot finds the Android SDK + keystore paths
if [ -f "$HOME/.config/godot/editor_settings-4.6.tres" ]; then
  mkdir -p "$XDG_CONFIG_HOME/godot"
  cp "$HOME/.config/godot/editor_settings-4.6.tres" "$XDG_CONFIG_HOME/godot/"
fi
mkdir -p export
"$GODOT_SAFE_BIN" --headless $MODE "Android" "$OUT"
